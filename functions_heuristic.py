...
def predict_fp(
    ...
    section = cast(Flight, flight.before(trou.start, strict=False)).last(minutes=20)
    assert section is not None
    gs = section.groundspeed_mean * 0.514444  # conversion to m/s

    data_points["latitude"].append(
        cast(Position, cast(Flight, flight.before(trou.start)).at_ratio(1)).latitude
    )
    data_points["longitude"].append(
        cast(Position, cast(Flight, flight.before(trou.start)).at_ratio(1)).longitude
    )
    data_points["timestamp"].append(
        cast(Position, cast(Flight, flight.before(trou.start)).at_ratio(1)).timestamp
    )
